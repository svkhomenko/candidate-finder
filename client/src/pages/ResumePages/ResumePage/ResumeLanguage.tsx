...
import { useDeleteResumeLanguageLevelMutation } from '~/store/api/resume-slice';
import ResumeLanguageUpdate from './ResumeLanguageUpdate';
import type { ResumeLanguageLevel, Resume } from '~/types/resume';
...
const ResumeLanguage = ({ languageLevel }: IProps) => {
  const [isEdit, setIsEdit] = useState(false);

  const [deleteLanguageLevel] = useDeleteResumeLanguageLevelMutation();

  const { handler: deleteHandler } = useRequestHandler<Pick<Resume, 'id'> & Pick<ResumeLanguageLevel, 'language'>>({
  ...
  return (
    <Card p="2" w="100%" variant="outline">
      {isEdit ? (
        <ResumeLanguageUpdate languageLevel={languageLevel} setIsEdit={setIsEdit} />
      ) : (
        <HStack spacing="4">
            ...
            <Menu>
              ...
              <MenuList>
                ...
                <MenuItem
                  color="red"
                  icon={<FiTrash2 />}
                  onClick={() => deleteHandler({ id: languageLevel.resumeId, language: languageLevel.language })}
                >
                  Видалити
                </MenuItem>
              </MenuList>
